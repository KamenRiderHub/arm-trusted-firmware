...
void bl31_main(void)
{
...
#endif
	//ATF_BP(" Clean ESR_EL3 [bl31_main]\n");
	//atf_testpoint_print_WriteESR();
	////atf_testpoint_print_RegGroup3();

	/* Perform platform setup in BL31 */
	ATF_BP(" START: bl31_platform_setup(); [bl31_main]\n");

	bl31_platform_setup();
	//printf(" Read 0x40000000 in bl1_setup is 0x%lx[bl1_main]\n",mmio_read_64(0x40000000));
	// printf(" Read 0xf7990000 in bl31_setup is 0x%lx[bl31_main]\n",mmio_read_64(0xf7990000));
	// printf(" Read 0x40000144 in bl31_setup is 0x%lx[bl31_main]\n",mmio_read_64(0x40000144));



	ATF_BP(" FINISH: bl31_platform_setup(); [bl31_main]\n");
	//atf_testpoint_print_RegGroup3();

	ATF_BP(" USE_GIC_DRIVER is %d [bl31_main]\n", USE_GIC_DRIVER);
	...
	ATF_BP(" core_pos = plat_my_core_pos(); is %d [bl31_main]\n", core_pos);

	ATF_BP(" START: gic_init(core_pos); [bl31_main]\n");

	gic_init(core_pos);

	ATF_BP(" FINISH: gic_init(core_pos); [bl31_main]\n");
	//atf_testpoint_print_RegGroup3();

	ATF_BP(" START: gic_pcpu_init(core_pos); [bl31_main]\n");

	gic_pcpu_init(core_pos);

	ATF_BP(" FINISH: gic_pcpu_init(core_pos); [bl31_main]\n");
	//atf_testpoint_print_RegGroup3();

	ATF_BP(" START: gic_cpuif_enable(core_pos); [bl31_main]\n");

	gic_cpuif_enable(core_pos);

	ATF_BP(" FINISH: gic_cpuif_enable(core_pos); [bl31_main]\n");
	//atf_testpoint_print_RegGroup3();

#endif /* USE_GIC_DRIVER */

	/* Initialise helper libraries */
	ATF_BP(" START: bl31_lib_init(); [bl31_main]\n");

	bl31_lib_init();

	ATF_BP(" FINISH: bl31_lib_init(); [bl31_main]\n");
	//atf_testpoint_print_RegGroup3();

	ATF_BP(" EL3_EXCEPTION_HANDLING is %d [bl31_main]\n", EL3_EXCEPTION_HANDLING);
	 ...
	 */
	ATF_BP(" START: bl31_prepare_next_image_entry(); [bl31_main]\n");
	
	bl31_prepare_next_image_entry();

	ATF_BP(" security_state is %lx\n",GET_SECURITY_STATE((bl31_plat_get_next_image_ep_info(bl31_get_next_image_type()))->h.attr));
	//atf_testpoint_print_RegGroup3();

	ATF_BP(" FINISH: bl31_prepare_next_image_entry(); [bl31_main]\n");
	 ...
	 * from BL31
	 */
	ATF_BP(" START: bl31_plat_runtime_setup(); [bl31_main]\n");

	bl31_plat_runtime_setup();

	ATF_BP(" security_state is %lx\n",GET_SECURITY_STATE((bl31_plat_get_next_image_ep_info(bl31_get_next_image_type()))->h.attr));

	ATF_BP(" FINISH: bl31_plat_runtime_setup(); [bl31_main]\n");
	...
	ATF_BP(" START: console_switch_state(CONSOLE_FLAG_RUNTIME); [bl31_main]\n");
	ATF_BP(" CONSOLE_FLAG_RUNTIME is %d [bl31_main]\n", CONSOLE_FLAG_RUNTIME);
	
	console_switch_state(CONSOLE_FLAG_RUNTIME);

	ATF_BP(" FINISH: console_switch_state(CONSOLE_FLAG_RUNTIME); [bl31_main]\n");
	ATF_BP(" security_state is %lx\n",GET_SECURITY_STATE((bl31_plat_get_next_image_ep_info(bl31_get_next_image_type()))->h.attr));

	// atf_testpoint_printMEM_32B(0xc8000000);
    // mmio_write_64(0xc8000000,0x123456789abcdef0);
    // mmio_write_64(0xc8000008,0x123456789abcdef0);
	// atf_testpoint_printMEM_32B(0xc8000000);

	// atf_testpoint_printMEM_32B(0x85000000);
	// atf_testpoint_printMEM_32B(0x88500000);
	// atf_testpoint_printMEM_32B(0xf0000000);

	ATF_BP(" CurrentEL = %lu\n", GET_EL(read_CurrentEl()));
	ATF_BP(" SP_EL1 = 0x%lx\n", read_sp_el1());
	ATF_BP(" SP_EL2 = 0x%lx\n", read_sp_el2());
	ATF_BP(" ELR_EL3 = 0x%lx\n", read_elr_el3());
	ATF_BP(" ELR_EL2 = 0x%lx\n", read_elr_el2());
	ATF_BP(" ELR_EL1 = 0x%lx\n", read_elr_el1());
	ATF_BP(" ELR_EL3 = 0x%lx\n", read_spsr_el3());
	ATF_BP(" ELR_EL2 = 0x%lx\n", read_spsr_el2());
	ATF_BP(" ELR_EL1 = 0x%lx\n", read_spsr_el1());
	...
	ATF_BP(" -- FINISH [bl31_main]--\n");
	NOTICE("FINISH bl31_main. Time to start UEFI\n");
}
...
void __init bl31_prepare_next_image_entry(void)
{
	...
	ATF_BP(" START: print_entry_point_info(next_image_info); [bl31_main]\n");
	print_entry_point_info(next_image_info);
	ATF_BP(" FINISH: print_entry_point_info(next_image_info); [bl31_main]\n");

	ATF_BP(" START: cm_init_my_context(next_image_info); [bl31_main]\n");

	cm_init_my_context(next_image_info);
	ATF_BP(" FINISH: cm_init_my_context(next_image_info); [bl31_main]\n");
