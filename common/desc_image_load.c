...
int get_bl_params_node_index(unsigned int image_id)
{
	...
	ATF_BP(" START: assert(image_id != INVALID_IMAGE_ID),image_id is %d, INVALID_IMAGE_ID is %d) \n",image_id,INVALID_IMAGE_ID);
	assert(image_id != INVALID_IMAGE_ID);
	ATF_BP(" FINISH: assert(image_id != INVALID_IMAGE_ID)\n");
	ATF_BP(" bl_mem_params_desc_num is %d\n",bl_mem_params_desc_num);
...
bl_mem_params_node_t *get_bl_mem_params_node(unsigned int image_id)
{
	...
	ATF_BP(" START: index = get_bl_params_node_index(image_id);\n");

	index = get_bl_params_node_index(image_id);

	ATF_BP(" index is %d\n",index);
	ATF_BP(" FINISH: index = get_bl_params_node_index(image_id);\n");
