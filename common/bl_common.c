...
static int load_image(unsigned int image_id, image_info_t *image_data)
{
	...
	ATF_BP(" [READY] io_read(image_handle, image_base, image_size, &bytes_read); [bl_common]\n");
	...
	for(int i=0;i<16;i++)
	{
		printf("%02x ",((uint8_t*)image_base)[i]);
	}printf("\n");
	ATF_BP(" ---------------- [PrintMemory] ---------------- \n");
	*/
exit:
...
void print_entry_point_info(const entry_point_info_t *ep_info)
{
	...
	INFO("SPSR = 0x%x\n", ep_info->spsr);
	// printf("PC = 0x%lx\n", ep_info->pc);
	// printf("param_header->type = 0x%x\n", ep_info->h.type);
	// printf("param_header->version = 0x%x\n", ep_info->h.version);
	// printf("param_header->size = 0x%x\n", ep_info->h.size);
	// printf("param_header->attr = 0x%x\n", ep_info->h.attr);
