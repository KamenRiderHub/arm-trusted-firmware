...
void generic_delay_timer_init_args(uint32_t mult, uint32_t div)
{
	...
	ATF_BP(" START: timer_init(&ops); in generic_delay_timer_init_args [generic_delay_timer]\n");

	timer_init(&ops);

	ATF_BP(" FINISH: timer_init(&ops); in generic_delay_timer_init_args [generic_delay_timer]\n");


	VERBOSE("Generic delay timer configured with mult=%u and div=%u\n",
		mult, div);
}
//【改】TODO：修改mult和div，尤其是注意plat_get_syscnt_freq2();
void generic_delay_timer_init(void)
{
	...
	ATF_BP(" START: generic_delay_timer_init_args(mult, div); in generic_delay_timer_init [generic_delay_timer]\n");
