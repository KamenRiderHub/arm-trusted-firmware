...
void __init gicv3_driver_init(const gicv3_driver_data_t *plat_driver_data)
{
	...
	//【改】gic_version靠这个读到的是0，我觉得应该模仿gicd_read_pidr2的mmio_read_32(base + GICD_PIDR2_GICV3);写一个写4进去的
	// gic_version = ARCH_REV_GICV4
	/* Check GIC version */
...
void gicv3_rdistif_init(unsigned int proc_num)
{
	...
	ATF_BP(" ctlr = gicd_read_ctlr(gicv3_driver_data->gicd_base); is 0x%x[gicv3_main]\n",ctlr);
	ATF_BP(" gicv3_driver_data->gicd_base is 0x%lx[gicv3_main]\n",gicv3_driver_data->gicd_base);
