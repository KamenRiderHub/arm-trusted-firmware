...
static int fip_dev_init(io_dev_info_t *dev_info, const uintptr_t init_params)
{
	...
	ATF_BP(" [READY] io_read(backend_handle, (uintptr_t)&header, sizeof(header), &bytes_read); [io_fip]\n");
			 ...
			 io_entity_t *entity)
{
	...
	found_file = 0;
	do {
		ATF_BP(" [READY] io_read(backend_handle, (uintptr_t)&current_fip_file.entry, sizeof(current_fip_file.entry), &bytes_read); [io_fip]\n");
			  ...
			  size_t *length_read)
{
	...
	ATF_BP(" [READY] io_read(backend_handle, buffer, length, &bytes_read); [io_fip]\n");
