...
static int memmap_block_read(io_entity_t *entity, uintptr_t buffer,
			     size_t length, size_t *length_read)
{
	ATF_BP(" ------------------ [WORKING: memmap_block_read] ------------------ \n");
	...
	*length_read = length;
	...
	/* 打印读取到的内容（原始部分） */
/*
	ATF_BP(" Read 32 bytes from (fp->base + fp->file_pos) 0x%llx:\n", (fp->base + fp->file_pos));
	ATF_BP(" Hex dump: ");
	for (int i = 0; i < 32; i++) {
		// 以十六进制打印每个字节，不足2位补0
		printf("%02x ", ((unsigned char *)((uintptr_t)(fp->base + fp->file_pos)))[i]);
		// 每16字节换行，提高可读性
		if ((i + 1) % 16 == 0) {
			printf("\nATF_BP: Hex dump: ");
		}
	...
	printf("\n");  // 结束当前行
*/
	// // 可选：同时打印ASCII字符（非可打印字符用.代替）
	// ATF_BP(" ASCII  : ");
	// for (int i = 0; i < 16; i++) {
	// 	unsigned char c = ((unsigned char *)((uintptr_t)(fp->base + fp->file_pos)))[i];
	...
	/* 打印读取到的内容（新增部分） */
/*
	ATF_BP(" Read 32 bytes from buffer 0x%lx:\n", buffer);
	ATF_BP(" Hex dump: ");
	for (int i = 0; i < 32; i++) {
		// 以十六进制打印每个字节，不足2位补0
