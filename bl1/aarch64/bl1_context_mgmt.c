...
{
	ATF_BP(" Here is *cm_get_context [bl1_context_mgmt]\n");
	ATF_BP(" security_state is 0x%x\n",security_state);
    // asm volatile (
    //     "adr	x0, .\n"
...
void bl1_prepare_next_image(unsigned int image_id)
{
	...
	ATF_BP(" security_state is 0x%x in bl1_prepare_next_image [bl1_context_mgmt]\n",security_state);
	...
	//atf_testpoint_print_RegGroup2();
	
	ATF_BP(" START: bl1_plat_set_ep_info(image_id, next_bl_ep); in bl1_prepare_next_image [bl1_context_mgmt]\n");
	/* Allow platform to make change */
	bl1_plat_set_ep_info(image_id, next_bl_ep);
	ATF_BP(" image_id is %d in bl1_prepare_next_image [bl1_context_mgmt]\n",image_id);

	ATF_BP(" FINISH: bl1_plat_set_ep_info(image_id, next_bl_ep); in bl1_prepare_next_image [bl1_context_mgmt]\n");
	//atf_testpoint_print_RegGroup2();
	print_entry_point_info(next_bl_ep);

	/* Prepare the context for the next BL image. */
	ATF_BP(" START: cm_init_my_context(next_bl_ep); in bl1_prepare_next_image [bl1_context_mgmt]\n");
	
	cm_init_my_context(next_bl_ep);
	ATF_BP(" FINISH: cm_init_my_context(next_bl_ep); in bl1_prepare_next_image [bl1_context_mgmt]\n");
	ATF_BP(" security_state is 0x%x in bl1_prepare_next_image [bl1_context_mgmt](2)\n",security_state);
	print_entry_point_info(next_bl_ep);
