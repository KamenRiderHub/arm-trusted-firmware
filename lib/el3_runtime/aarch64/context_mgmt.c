...
{
	ATF_BP(" Here is setup_ns_context\n");
	print_entry_point_info(ep);
	...
	print_entry_point_info(ep);
	ATF_BP(" FINISH: setup_ns_context\n");
...
static void setup_context_common(cpu_context_t *ctx, const entry_point_info_t *ep)
{
	...
	state = get_el3state_ctx(ctx);
 	ATF_BP(" state = get_el3state_ctx(ctx); is %p [context_mgmt]\n",&state);
	 ...
	 * It is assu	med that an all-zero lower-EL context is good enough for it
...
void cm_setup_context(cpu_context_t *ctx, const entry_point_info_t *ep)
{
	...
	ATF_BP(" Here is cm_setup_context [context_mgmt]\n");
	 ...
	 */
	ATF_BP(" START: setup_context_common(ctx, ep); [context_mgmt]\n");

	setup_context_common(ctx, ep);
	ATF_BP(" FINISH: setup_context_common(ctx, ep); [context_mgmt]\n");

	security_state = GET_SECURITY_STATE(ep->h.attr);
	ATF_BP(" security_state is %x [context_mgmt]\n",security_state);
	...
	ATF_BP(" FINISH cm_setup_context [context_mgmt]\n");
...
{
	cpu_context_t *ctx;
	ATF_BP(" START: ctx = cm_get_context(GET_SECURITY_STATE(ep->h.attr)); [context_mgmt]\n");
	print_entry_point_info(ep);
	ctx = cm_get_context(GET_SECURITY_STATE(ep->h.attr));

	ATF_BP(" FINISH: ctx = cm_get_context(GET_SECURITY_STATE(ep->h.attr)); [context_mgmt]\n");

	ATF_BP(" ep->h.attr is %x [context_mgmt]\n",ep->h.attr);
	
	ATF_BP(" ctx = cm_get_context(GET_SECURITY_STATE(ep->h.attr)); is %p [context_mgmt]\n",&ctx);

	ATF_BP(" START: cm_setup_context(ctx, ep); [context_mgmt]\n");

	cm_setup_context(ctx, ep);

	ATF_BP(" FINISH: cm_setup_context(ctx, ep); [context_mgmt]\n");
}
...
{
	ATF_BP(" Here is cm_prepare_el3_exit [context_mgmt]\n");
	ATF_BP(" security_state is 0x%x in cm_prepare_el3_exit [context_mgmt]\n",security_state);
