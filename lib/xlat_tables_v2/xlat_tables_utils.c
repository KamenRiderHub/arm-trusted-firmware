	...
	while (table_idx < table_entries) {
		...
		if ((desc & DESC_MASK) == INVALID_DESC) {
			...
			if (((desc & DESC_MASK) == TABLE_DESC) &&
				 ...
				 */
				printf("%sVA:0x%lx size:0x%zx [%p]\n",
				       level_spacers[level],
				       table_idx_va, level_size, (void*)&table_base[table_idx]);
				...
				printf("%sVA:0x%lx PA:0x%" PRIx64 " size:0x%zx [%p]",
				       level_spacers[level], table_idx_va,
				       (uint64_t)(desc & TABLE_ADDR_MASK),
				       level_size, (void*)&table_base[table_idx]);
...
void xlat_tables_print(xlat_ctx_t *ctx)
{
	...
	printf("Translation tables state:\n");
	printf("  Xlat regime:     EL%s\n", xlat_regime_str);
	printf("  Max allowed PA:  0x%llx\n", ctx->pa_max_address);
	printf("  Max allowed VA:  0x%lx\n", ctx->va_max_address);
	printf("  Max mapped PA:   0x%llx\n", ctx->max_pa);
	printf("  Max mapped VA:   0x%lx\n", ctx->max_va);

	printf("  Initial lookup level: %u\n", ctx->base_level);
	printf("  Entries @initial lookup level: %u\n",
...
#endif
	printf("  Used %d sub-tables out of %d (spare: %d)\n",
