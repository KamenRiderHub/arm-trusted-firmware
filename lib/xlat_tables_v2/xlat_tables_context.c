 ...
 * currently executing.
 【注】&tf_xlat_ctx来源于“REGISTER_XLAT_CONTEXT(tf, MAX_MMAP_REGIONS, MAX_XLAT_TABLES,
		      PLAT_VIRT_ADDR_SPACE_SIZE, PLAT_PHY_ADDR_SPACE_SIZE);”，注意这里的“tf”
 */
...
{
	ATF_BP(" START: setup_mmu_cfg(); in enable_mmu_el3 [xlat_tables_context]\n");
	ATF_BP(" &mmu_cfg_params = %p\n", (uint64_t *)&mmu_cfg_params);
	ATF_BP(" mmu_cfg_params[0] = %lx\n", mmu_cfg_params[0]);
	ATF_BP(" mmu_cfg_params[1] = %lx\n", mmu_cfg_params[1]);
	ATF_BP(" mmu_cfg_params[2] = %lx\n", mmu_cfg_params[2]);



	ATF_BP(" flags = %u\n", flags);
	ATF_BP(" tf_xlat_ctx.base_table = %p\n", tf_xlat_ctx.base_table);
	ATF_BP(" MAX_PHYS_ADDR = %llx\n", MAX_PHYS_ADDR);
	ATF_BP(" tf_xlat_ctx.va_max_address = %ld\n", tf_xlat_ctx.va_max_address);
	ATF_BP(" EL3_REGIME = %u\n", EL3_REGIME);
	...
	for(int i=0;i<5;i++)
	{
		ATF_BP(" tf_xlat_ctx.base_table[%d] = 0x%lx\n", i, tf_xlat_ctx.base_table[i]);
	}
	
	
	ATF_BP(" mmu_cfg_params[0] = 0x%lx\n", mmu_cfg_params[0]);
	ATF_BP(" mmu_cfg_params[1] = 0x%lx\n", mmu_cfg_params[1]);
	ATF_BP(" mmu_cfg_params[2] = 0x%lx\n", mmu_cfg_params[2]);
	ATF_BP(" FINISH: setup_mmu_cfg(); in enable_mmu_el3 [xlat_tables_context]\n");

	// flush_dcache_range(BL_CODE_BASE, BL1_CODE_END - BL_CODE_BASE);
	// flush_dcache_range(0x85df5000, 0x85f00000 - 0x85df5000);


	ATF_BP(" START: enable_mmu_direct_el3(); in enable_mmu_el3 [xlat_tables_context]\n");
	enable_mmu_direct_el3(flags);
	ATF_BP(" FINISH: enable_mmu_direct_el3(); in enable_mmu_el3 [xlat_tables_context]\n");
