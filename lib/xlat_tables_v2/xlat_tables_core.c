...
void mmap_add_ctx(xlat_ctx_t *ctx, const mmap_region_t *mm)
{
	...
	while (mm_cursor->granularity != 0U) {
		...
		ATF_BP(" ----------Print mm_cursor one by one [%d]----------\n",tmp);
		ATF_BP(" mm_cursor->base_pa is: %llx\n",mm_cursor->base_pa);
		ATF_BP(" mm_cursor->base_va is: %lx\n",mm_cursor->base_va);
		ATF_BP(" mm_cursor->size is: %lx\n",mm_cursor->size);
		ATF_BP(" mm_cursor->attr is: %x\n",mm_cursor->attr);
		ATF_BP(" mm_cursor->granularity is: %lx\n",mm_cursor->granularity);
...
void __init init_xlat_tables_ctx(xlat_ctx_t *ctx)
{
	...
	ATF_BP(" Using xlat_mmap_print(mm):\n");
	xlat_mmap_print(mm);
