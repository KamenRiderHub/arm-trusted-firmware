...
#endif 
 */
//【改】已经在对应文件下改了ARM_TRUSTED_SRAM_BASE
 //ARM_TRUSTED_SRAM_BASE->DRAM--8000 0000
 ...
 */
//【改】改基地址cx芯片上对应位置在memory（DRAM）
//#define PLAT_ARM_TRUSTED_DRAM_BASE	UL(0x06000000)
#define PLAT_ARM_TRUSTED_DRAM_BASE	(UL(0x06000000) + CX_DRAM_BASE)
 ...
 * 用于隔离安全和非安全世界的内存访问。
  * ！！！这个东西定义了但没被使用（#if defined(PLAT_XLAT_TABLES_DYNAMIC)才行）
 */
...
便于启动流程的管理和内存布局的规划。
 */
// // 【改】改基地址cx芯片上对应位置在memory（DRAM）【改2】错了，不要偏移
// #define PLAT_ARM_NS_IMAGE_BASE		(ARM_DRAM1_BASE + UL(0x8000000) + CX_DRAM_BASE)
#define PLAT_ARM_NS_IMAGE_BASE		(ARM_DRAM1_BASE + UL(0x8200000))
...
/*
以上内容，综合如下：
在bl31中，PLAT_ARM_MMAP_ENTRIES为9，MAX_XLAT_TABLES为7
在bl32中，PLAT_ARM_MMAP_ENTRIES为9，MAX_XLAT_TABLES为6
在bl2中，PLAT_ARM_MMAP_ENTRIES为12，MAX_XLAT_TABLES为5
在bl1中，PLAT_ARM_MMAP_ENTRIES为12，MAX_XLAT_TABLES为5【#elif !USE_ROMLIB里的# else】
*/
...
1.如果启用了TF_MBEDTLS_KEY_ALG_ID == TF_MBEDTLS_RSA_AND_ECDSA（即同时支持RSA和ECDSA算法），或
PSA_CRYPTO（启用PSA加密API）为1，或FVP_TRUSTED_SRAM_SIZE == 512（受信任SRAM大小为512KB），
...
【PSA_CRYPTO】在make_helpers\defaults.mk：
...
//【所以 PLAT_ARM_MAX_BL2_SIZE 最终用的是上面这最后的定义】
...
//【所以 PLAT_ARM_MAX_BL31_SIZE 最终用的是上面这最后的定义】
...
//【所以 PLAT_ARM_MAX_BL32_SIZE 最终用的是上面这最后的定义】
...
# else
#  define PLATFORM_STACK_SIZE		UL(0x500) // BL1阶段的堆栈大小为1280字节（0x500）
# endif /* CRYPTO_SUPPORT */
#elif defined(IMAGE_BL2)
# if CRYPTO_SUPPORT
#  define PLATFORM_STACK_SIZE		UL(0x1000)
# else
#  define PLATFORM_STACK_SIZE		UL(0x600) // BL2阶段的堆栈大小为1536字节（0x600）
# endif /* CRYPTO_SUPPORT */
...
#  define PLATFORM_STACK_SIZE		UL(0x1000)
# else
#  define PLATFORM_STACK_SIZE		UL(0x800) // BL31阶段的堆栈大小为2048字节（0x800）
# endif /* DRTM_SUPPORT */
...
【改】已经在v2m_def.h里改了V2M_FLASH0_BASE、V2M_IOFPGA_BASE
*/
...
//【注】"PL011 related constants"需要结合泽鹏的uart驱动决定“PL011 related constants”怎么处理
//【注】cx芯片uart所在的那一大块外设区（“……其它所有寄存器……”）已通过修改V2M_IOFPGA_BASE，影响V2M_MAP_IOFPGA，然后给mmu
/*
...
//【改】cx芯片的SMMUV3基地址是0x0D00_0000，“PLAT_ARM_SMMUV3_ROOT_REG_OFFSET”是在启动RME的时候会用到，我们不用管
// #define PLAT_FVP_SMMUV3_BASE		UL(0x2b400000)
#define PLAT_FVP_SMMUV3_BASE		UL(0x0D000000)
#define PLAT_ARM_SMMUV3_ROOT_REG_OFFSET UL(0x20000)

//【注】通过注释掉使用它们的函数避免了使用cci和ccn这些定义
...
//【改】通过注释掉“arm_configure_sys_timer();”避免使用它
/* System timer related constants */
#define PLAT_ARM_NSTIMER_FRAME_ID	U(1)

//【注】不用管，它被“plat\arm\common\arm_pm.c”文件的plat_arm_program_trusted_mailbox使用，只有当启用了EL3_PAYLOAD_BASE（默认不用）才会引入这个文件
/* Mailbox base address */
#define PLAT_ARM_TRUSTED_MAILBOX_BASE	ARM_TRUSTED_SRAM_BASE

//【注】按atf参考代码的思路以cx芯片的实际地址来看，暂时不用改region 1
 ...
 */
//【改】按atf参考代码的思路以cx芯片的实际地址来看，这里需要改到0x01_0000_0000
// #define PLAT_ARM_PCI_MEM_2_BASE		(ULL(0x4000000000))
#define PLAT_ARM_PCI_MEM_2_BASE		(ULL(0x100000000))
 ...
 */
//【注】不管，在需要ARM_CONFIG_HAS_TZC时才会用到以下TZC定义（用上TrustZone® Address Space Controller, TZC-400）
