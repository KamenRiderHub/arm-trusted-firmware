		...
		u_register_t arg1, u_register_t arg2, u_register_t arg3)
{
	...
	ATF_BP(" soc_fw_config_info->config_addr is %lx \n",soc_fw_config_info->config_addr);
	ATF_BP(" soc_fw_config_info->config_max_size is %x \n",soc_fw_config_info->config_max_size);
	ATF_BP(" soc_fw_config_info->config_id is %d \n",soc_fw_config_info->config_id);
	ATF_BP(" soc_fw_config_info->secondary_config_addr is %lx \n",soc_fw_config_info->secondary_config_addr);
	ATF_BP(" SOC_FW_CONFIG_ID is %d \n",SOC_FW_CONFIG_ID);
	ATF_BP(" ---------------- [PrintMemory] ---------------- \n");
	// for(int i=0;i<16;i++)
	// {
	// 	printf("%02x ",((uint8_t*)soc_fw_config_info->config_addr)[i]);
	// }printf("\n");
	ATF_BP(" ---------------- [PrintMemory] ---------------- \n");
	/*
	 ...
	 */
	ATF_BP(" START: hw_config_info = FCONF_GET_PROPERTY(dyn_cfg, dtb, HW_CONFIG_ID); in bl31_plbl31_early_platform_setup2at_arch_setup [fvp_bl31_setup]\n");
	...
	ATF_BP(" hw_config_info->config_addr is %lx \n",hw_config_info->config_addr);
	ATF_BP(" hw_config_info->config_max_size is %x \n",hw_config_info->config_max_size);
	ATF_BP(" hw_config_info->config_id is %d \n",hw_config_info->config_id);
	ATF_BP(" hw_config_info->secondary_config_addr is %lx \n",hw_config_info->secondary_config_addr);
	ATF_BP(" HW_CONFIG_ID is %d \n",HW_CONFIG_ID);
	ATF_BP(" ---------------- [PrintMemory] ---------------- \n");
	// for(int i=0;i<16;i++)
	// {
	// 	printf("%02x ",((uint8_t*)hw_config_info->config_addr)[i]);
	// }printf("\n");
	ATF_BP(" ---------------- [PrintMemory] ---------------- \n");
	...
	//【改】注释掉bl31的 fvp_interconnect_init(); fvp_interconnect_enable(); fvp_timer_init();
	/*
...
{
	ATF_BP(" START: arm_bl31_platform_setup(); in bl31_platform_setup [fvp_bl31_setup]\n");

	arm_bl31_platform_setup();

	ATF_BP(" FINISH: arm_bl31_platform_setup(); in bl31_platform_setup [fvp_bl31_setup]\n");


#if USE_GIC_DRIVER == 3
	ATF_BP(" START: fvp_pcpu_init(); in bl31_platform_setup [fvp_bl31_setup]\n");

	fvp_pcpu_init();
	ATF_BP(" FINISH: fvp_pcpu_init(); in bl31_platform_setup [fvp_bl31_setup]\n");

	ATF_BP(" START: fvp_gic_driver_pre_init(); in bl31_platform_setup [fvp_bl31_setup]\n");
	
	fvp_gic_driver_pre_init();
	ATF_BP(" FINISH: fvp_gic_driver_pre_init(); in bl31_platform_setup [fvp_bl31_setup]\n");
...
void __init bl31_plat_arch_setup(void)
{
	...
	ATF_BP(" START: fconf_populate(HW_CONFIG, hw_config_info->config_addr); in bl31_plat_arch_setup [fvp_bl31_setup]\n");

	/* Populate HW_CONFIG device tree with the mapped address */
	fconf_populate("HW_CONFIG", hw_config_info->config_addr);
	ATF_BP(" FINISH: fconf_populate(HW_CONFIG, hw_config_info->config_addr); in bl31_plat_arch_setup [fvp_bl31_setup]\n");
...
//【改】bl1的时候plat_get_syscnt_freq2是在arm_common，bl31在这，需要像当时那样改（直接赋值）
unsigned int plat_get_syscnt_freq2(void)
{
	ATF_BP(" START: plat_get_syscnt_freq2 in [fvp_bl31_setup]\n");
	...
	//unsigned int syscnt_freq2 = 12.5*1000*1000;

	counter_base_frequency = syscnt_freq2;
	ATF_BP(" FINISH: plat_get_syscnt_freq2 in [fvp_bl31_setup]\n");
	ATF_BP(" counter_base_frequency = %u \n", counter_base_frequency);
