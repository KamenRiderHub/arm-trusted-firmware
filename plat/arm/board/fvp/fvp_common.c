...
void __init fvp_config_setup(void)
{
	...
	//【改】直接给arm_config.flags赋值，只保留我们需要或可能需要的内容
	//ARM_CONFIG_BASE_MMAP、ARM_CONFIG_FVP_HAS_SMMUV3、ARM_CONFIG_FVP_SHIFTED_AFF
	//我们用的SMMUv3.2是SMMUv3的子版本
	//ARM_CONFIG_FVP_SHIFTED_AFF负责MPIDR_EL1 的 MT（MultiThread）位，判断平台是否采用“移位亲和性”（Shifted Affinity）结构
	arm_config.flags = 0;
	arm_config.flags |= (ARM_CONFIG_BASE_MMAP | ARM_CONFIG_FVP_HAS_SMMUV3);
	if ((read_mpidr_el1() & MPIDR_MT_MASK) != 0U)
		arm_config.flags |= ARM_CONFIG_FVP_SHIFTED_AFF;
}
...
//【改0923】注释掉USE_SP804_TIMER
// TODO：可能需要注释掉使能（scp做了，且nrd的NRD_CSS_GENERIC_REFCLK_BASE也不靠谱）
...
// #else
	ATF_BP(" START: generic_delay_timer_init(); in fvp_timer_init [fvp_common]\n");

	generic_delay_timer_init();

	ATF_BP(" FINISH: generic_delay_timer_init(); in fvp_timer_init [fvp_common]\n");
