...
void arm_bl2_dyn_cfg_init(void)
{
	...
	ATF_BP(" ARRAY_SIZE(config_ids) is %ld [arm_dyn_cfg]\n",ARRAY_SIZE(config_ids));
	ATF_BP(" START: Iterate through all the fw config IDs [arm_dyn_cfg]\n");

	/* Iterate through all the fw config IDs */
	for (i = 0; i < ARRAY_SIZE(config_ids); i++) {
		/* Get the config load address and size */

		ATF_BP(" ----------------Get the config load address and size [%d]----------------\n",i);
		ATF_BP(" config_ids[%d] is %d\n",i,config_ids[i]);
		ATF_BP(" START: get_bl_mem_params_node(config_ids[%d]) [arm_dyn_cfg]\n",i);

		cfg_mem_params = get_bl_mem_params_node(config_ids[i]);
		if (cfg_mem_params == NULL) {
			ATF_BP(" [NOTE] cfg_mem_params == NULL\n");
		...
		ATF_BP(" FINISH: get_bl_mem_params_node(config_ids[%d]) [arm_dyn_cfg]\n",i);

		ATF_BP(" START: FCONF_GET_PROPERTY(dyn_cfg, dtb, config_ids[i]) [arm_dyn_cfg]\n");
		...
		ATF_BP(" FINISH: FCONF_GET_PROPERTY(dyn_cfg, dtb, config_ids[i]) [arm_dyn_cfg]\n");

		image_base = dtb_info->config_addr;
		image_size = dtb_info->config_max_size;

		ATF_BP(" image_base = dtb_info->config_addr is %lx \n",image_base);
		ATF_BP(" image_size = dtb_info->config_max_size is %x \n",image_size);



		ATF_BP(" [JUDGE](config_ids[i] != HW_CONFIG_ID) is %d \n",(config_ids[i] != HW_CONFIG_ID));
		...
		if (config_ids[i] != HW_CONFIG_ID) {
			...
			ATF_BP(" START: [JUDGE](check_uptr_overflow(image_base, image_size))\n");
			...
			if (check_uptr_overflow(image_base, image_size)) {
				VERBOSE("%s=%d as its %s is overflowing uptr\n",
					...
					config_ids[i],
					"load-address");
				ATF_BP(" [BUG]%s=%d as its %s is overflowing uptr\n",
					"skip loading of firmware config",
					config_ids[i],
					"load-address");
				printf("check_uptr_overflow(image_base, image_size) is ((image_base) > (UINTPTR_MAX - (image_size)))\n");
				printf("UINTPTR_MAX is %ld",UINTPTR_MAX);
				printf("image_base is %ld",image_base);
				printf("image_size is %d",image_size);


				error_config_id = config_ids[i];
				ATF_BP(" [CHANGE]error_config_id is changed to %d as [check_uptr_overflow]\n",error_config_id);
				ATF_BP(" --------------------------------\n");

				continue;
			}

			ATF_BP(" FINISH: [JUDGE](check_uptr_overflow(image_base, image_size))\n");
			...
			if ((image_base >= BL31_BASE) &&
				...
				VERBOSE("%s=%d as its %s is overlapping BL31\n",
					...
					"load-address");
				error_config_id = config_ids[i];
				ATF_BP(" [CHANGE]error_config_id is changed to %d as [BL31_BASE]\n",error_config_id);
				ATF_BP(" --------------------------------\n");
				continue;
			}
#endif
			...
			if (image_base < ARM_BL_RAM_BASE) {
				VERBOSE("%s=%d as its %s is invalid\n",
					...
					"load-address");
				error_config_id = config_ids[i];
				ATF_BP(" [CHANGE]error_config_id is changed to %d as [ARM_BL_RAM_BASE]\n",error_config_id);
				ATF_BP(" --------------------------------\n");
			...
			if ((image_base >= BL32_BASE) &&
				...
				VERBOSE("%s=%d as its %s is overlapping BL32\n",
					...
					"load-address");
				error_config_id = config_ids[i];
				ATF_BP(" [CHANGE]error_config_id is changed to %d as [BL32_BASE]\n",error_config_id);
				ATF_BP(" --------------------------------\n");
				continue;
			}
#endif
