...
#define MAP_BL1_TOTAL		MAP_REGION_FLAT(			\
					...
					MT_MEMORY | MT_RW | MT_EXECUTE | EL3_PAS)
/*
#define MAP_BL1_TOTAL		MAP_REGION_FLAT(			\
					bl1_tzram_layout.total_base,	\
					bl1_tzram_layout.total_size,	\
...
#define MAP_BL1_TOTAL		MAP_REGION_FLAT(			\
					bl1_tzram_layout.total_base,	\
					bl1_tzram_layout.total_size,	\
					MT_NON_CACHEABLE | MT_RW | EL3_PAS)
*/
/*
...
#define MAP_BL1_RO		MAP_REGION_FLAT(			\
					BL_CODE_BASE,			\
					BL1_CODE_END - BL_CODE_BASE,	\
					(MT_NON_CACHEABLE | MT_RO | MT_EXECUTE) | EL3_PAS),		\
				MAP_REGION_FLAT(			\
					BL1_RO_DATA_BASE,		\
					BL1_RO_DATA_END			\
						- BL_RO_DATA_BASE,	\
					(MT_NON_CACHEABLE | MT_RO | MT_EXECUTE_NEVER) | EL3_PAS)
...
{
	// init_uart();
	//print_string("Test_point0222222");
	...
	// ATF_BP(" START: arm_console_boot_init in arm_bl1_early_platform_setup [arm_bl1_setup]\n");

	arm_console_boot_init();
	ATF_BP(" FINISH: arm_console_boot_init in arm_bl1_early_platform_setup [arm_bl1_setup]\n");
...
void arm_bl1_plat_arch_setup(void)
{
	...
	ATF_BP(" START: setup_page_tables(); in arm_bl1_plat_arch_setup [arm_bl1_setup]\n");

	setup_page_tables(bl_regions, plat_arm_get_mmap());


	printf("ATF_BP: [TEST_MAP] bl1_tzram_layout.total_base is 0x%lx\n", bl1_tzram_layout.total_base);
	printf("ATF_BP: [TEST_MAP] bl1_tzram_layout.total_size is 0x%lx\n", bl1_tzram_layout.total_size);
	printf("ATF_BP: [TEST_MAP] BL_CODE_BASE is 0x%lx\n", BL_CODE_BASE);
	printf("ATF_BP: [TEST_MAP] BL1_CODE_END is 0x%lx\n", BL1_CODE_END);
	printf("ATF_BP: [TEST_MAP] BL1_RO_DATA_BASE is 0x%lx\n", BL1_RO_DATA_BASE);
	printf("ATF_BP: [TEST_MAP] BL1_RO_DATA_END is 0x%lx\n", BL1_RO_DATA_END);

	printf("ATF_BP: [TEST_MAP] BL_CODE_BASE is 0x%lx\n", BL_CODE_BASE);
	printf("ATF_BP: [TEST_MAP] BL_CODE_END is 0x%lx\n", BL_CODE_END);
	// printf("ATF_BP: [TEST_MAP] BL1_CODE_BASE is 0x%lx\n", BL1_CODE_BASE);
	printf("ATF_BP: [TEST_MAP] BL1_CODE_END is 0x%lx\n", BL1_CODE_END);
	printf("ATF_BP: [TEST_MAP] BL_RO_DATA_BASE is 0x%lx\n", BL_RO_DATA_BASE);
	printf("ATF_BP: [TEST_MAP] BL_RO_DATA_END is 0x%lx\n", BL_RO_DATA_END);	
	printf("ATF_BP: [TEST_MAP] BL1_RO_DATA_BASE is 0x%lx\n", BL1_RO_DATA_BASE);
	printf("ATF_BP: [TEST_MAP] BL1_RO_DATA_END is 0x%lx\n", BL1_RO_DATA_END);
	printf("ATF_BP: [TEST_MAP] __TEXT_START__ is 0x%p\n", &__TEXT_START__);
	printf("ATF_BP: [TEST_MAP] __TEXT_END__ is 0x%p\n", &__TEXT_END__);	
	printf("ATF_BP: [TEST_MAP] __RODATA_START__ is 0x%p\n", &__RODATA_START__);
	printf("ATF_BP: [TEST_MAP] __RODATA_END__ is 0x%p\n", &__RODATA_END__);



	ATF_BP(" FINISH: setup_page_tables(); in arm_bl1_plat_arch_setup [arm_bl1_setup]\n");

	ATF_BP(" START: enable_mmu_el3(); in arm_bl1_plat_arch_setup [arm_bl1_setup]\n");
	printf("sctlr_el3 is 0x%lx[before]\n",read_sctlr_el3());
	write_sctlr_el3(read_sctlr_el3() & (~(SCTLR_A_BIT)));
	printf("sctlr_el3 is 0x%lx[after]\n",read_sctlr_el3());
#ifdef __aarch64__
	enable_mmu_el3(0);
#else
	...
	ATF_BP(" FINISH: enable_mmu_el3(); in arm_bl1_plat_arch_setup [arm_bl1_setup]\n");


	ATF_BP(" START: arm_setup_romlib(); in arm_bl1_plat_arch_setup [arm_bl1_setup]\n");

	arm_setup_romlib();

	ATF_BP(" FINISH: arm_setup_romlib(); in arm_bl1_plat_arch_setup [arm_bl1_setup]\n");
...
{
	ATF_BP(" Here is arm_bl1_platform_setup [arm_bl1_setup]\n");
	...
	if (config_info != NULL) {
		...
		/* load TB_FW_CONFIG */
		ATF_BP(" START: fconf_load_config(TB_FW_CONFIG_ID) in arm_bl1_platform_setup [arm_bl1_setup]\n");
		err = fconf_load_config(TB_FW_CONFIG_ID);
		if (err < 0) {
			ERROR("Loading of TB_FW_CONFIG failed %d\n", err);
			plat_error_handler(err);
		}
		ATF_BP(" FINISH: fconf_load_config(TB_FW_CONFIG_ID) in arm_bl1_platform_setup [arm_bl1_setup]\n");
	...
	ATF_BP(" START: bl1_plat_get_image_desc(BL2_IMAGE_ID) in arm_bl1_platform_setup [arm_bl1_setup]\n");
	desc = bl1_plat_get_image_desc(BL2_IMAGE_ID);
	ATF_BP(" FINISH: bl1_plat_get_image_desc(BL2_IMAGE_ID) in arm_bl1_platform_setup [arm_bl1_setup]\n");
...
//【改】注释掉操作timer的这个函数，timer是被scp管理的
// #ifdef ARM_SYS_TIMCTL_BASE
// 	arm_configure_sys_timer();
// #endif

//【改】这里需要改成直接给它赋值，值为系统计数器的实际频率(100MHz)
#if (ARM_ARCH_MAJOR > 7) || defined(ARMV7_SUPPORTS_GENERIC_TIMER)
	write_cntfrq_el0(plat_get_syscnt_freq2());
#endif
// unsigned int syscnt_freq2 = 100*1000*1000;
// #if (ARM_ARCH_MAJOR > 7) || defined(ARMV7_SUPPORTS_GENERIC_TIMER)
// 	write_cntfrq_el0(syscnt_freq2);
// #endif

	ATF_BP(" Here we finish arm_bl1_platform_setup [arm_bl1_setup]\n");
